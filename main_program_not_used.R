...
mc_complete2 <- function(mc_fun, from, to, by, summary_functions, input_var,
  ...
  if(plot==TRUE){
    plot1 <- plot_function(data_plot = df_sim, plot_lines=length(output_var))
  }
  ...
  if(summarise == TRUE){ #output with summary = TRUE
    if(plot==TRUE){
      return(list(summary_oop(summary_functions, df_sim, output_var), plot1))
    } else{
    ...
    if(plot==TRUE){
      return(list(knitr::kable(df_sim, row.names = FALSE), plot1))
    } else{
...
plot_function <- function(data_plot, plot_lines){
  if(plot_lines==1){
    plot1 <- ggplot(data=data_plot) + 
      geom_line(aes(x=data_plot[,1], y=data_plot[,2]), color="darkred") +
      xlab(names(data_plot)[1]) +
      ylab("magnitude")
    return(plot1)
  }
  ...
  if(plot_lines==2){
    plot2 <- ggplot(data=data_plot) + 
      geom_line(aes(x=data_plot[,1], y=data_plot[,2]), color="darkred") +
      geom_line(aes(x=data_plot[,1], y=data_plot[,3]), color="steelblue") +
      xlab(names(data_plot)[1])
    return(plot2)
  }  
  ...
  if(plot_lines==3){
    plot3 <- ggplot(data=data_plot) + 
      geom_line(aes(x=data_plot[,1], y=data_plot[,2]), color="darkred") +
      geom_line(aes(x=data_plot[,1], y=data_plot[,3]), color="steelblue") +
      geom_line(aes(x=data_plot[,1], y=data_plot[,4]), color="green") +
      xlab(names(data_plot)[1])
    return(plot3)
  }  
...
stringx <- names(data_plot[1])

ggplot()+
  geom_line(data=data_plot,aes(y=data_plot[,2],x= data_plot[,1],colour=do.call(stringx)),size=1 )+
  geom_line(data=data_plot,aes(y=data_plot[,3],x= data_plot[,1],colour="Y2"),size=1) +
  scale_color_manual(name = "Y series", values = c(do.call(stringx) = "darkblue", "Y2" = "red"))
