...
returns a probability distribution of sample which represents that of parameter[@Barbu_2022]. Monte Carlo Method is combined with programming in modern research and contributes to various studies in statistics, economics, and many other science fields. 
...
`create_grid` is one of the functions to improve performance of the main Monte Carlo simulation function. That creates a hyper-parameter grid with all permutations of the given parameters. Although a is mainly not known in advance, it is important to find an appropriate range of a to improve performance of  functions.[@Rana_2022]. Users can make their combination of hyper-parameters, then apply it into MC simulation in the main function.
...
`data_generation()` takes `grid` and `simulation` as inputs. Users can define a probability distribution for the data generation process by entering the name of an respective R function, f.e. the normal distribution(`rnorm`) or uniform distribution(`runif`).

The n data points created for each set of parameters get stored as separate elements in a list, since this format is a very flexible way of storing data. 

`data_generation()` chooses a mapping function itself based on the number of parameters. Regarding the number of parameters and if parallelization is chosen or not, different mapping functions of the `purrrr` package (without parallelization) or `furrr`-package (with parallelisation) are used. The table below gives an overview over the different constellations:

| Map | Map for Parallelization | Number of parameters |
| :------: | :------: | :------: |
| purrr:map() | furrr:future_map | $n = 1$ |
| purrr:map2() | furrr:future_map2 | $n = 2$ | 
| purrr:pmap | furrr:future_pmap | $n \geq 3$ | 
...
Monte Carlo simulations can become very demanding in terms of computing time. In that case, parallel processing can be useful. In the parallel processes, each process is executed simultaneously, but separately. Interconnections are proceeded through communication chanel [@Czech_2017]
...
`sim1` contains the generated data by `data_generation` with `grid2` and the poisson distribution as inputs. The below function shows the application of `data_generation` to uniform and poisson distribution:
...
`out$average` is for visualization purpose, since working with arrays sometimes are problematic. `ggplot2` can be used without any problem by taking average of the simulation.
