...
as inputs. Users can define a probability distribution for the data
generation process by entering the name of an respective R function,
f.e. the normal distribution(\texttt{rnorm}) or uniform
distribution(\texttt{runif}).
...
the number of parameters. Regarding the number of parameters and if
parallelization is chosen or not, different mapping functions of the
\texttt{purrrr} package (without parallelization) or
\texttt{furrr}-package (with parallelisation) are used. The table below
gives an overview over the different constellations:
...
\midrule()
\endhead
purrr:map() & furrr:future\_map & \(n = 1\) \\
purrr:map2() & furrr:future\_map2 & \(n = 2\) \\
purrr:pmap & furrr:future\_pmap & \(n \geq 3\) \\
\bottomrule()
\end{longtable}
...
time. In that case, parallel processing can be useful. In the parallel
processes, each process is executed simultaneously, but separately.
Interconnections are proceeded through communication chanel
\autocite{Czech_2017}
...
with \texttt{grid2} and the poisson distribution as inputs. The below
...
parameter constellation. Through \texttt{create\_array\_function}, users
can obtain all combinations corresponding each grid, the number of
repetition, and the summary function that users require, such as
...
##  Median  : 5 
## 
##  Execution Time of Monte Carlo Simulation 0.2354102 secs 
## 
...
\texttt{out\$average} is for visualization purpose, since working with
arrays sometimes are problematic. \texttt{ggplot2} can be used without
any problem by taking average of the simulation.
...
As given graphs above, the simulation result works well with ggplot2
methods.
...
##  Median  : 1.246948 
## 
##  Execution Time of Monte Carlo Simulation 0.1340079 secs 
## 
##  Name of The Class : Eco
\end{verbatim}
...
##  Median  : 1.284057 
## 
##  Execution Time of Monte Carlo Simulation 33.59968 secs 
## 
##  Name of The Class : Eco
\end{verbatim}
...
##  Median  : 1.284057 
## 
##  Execution Time of Monte Carlo Simulation 32.74619 secs 
## 
##  Name of The Class : Eco
\end{verbatim}
