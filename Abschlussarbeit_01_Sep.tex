...
defined to find \(beta2\) coefficients and also parameter list defined.
Here, the execution time of the \texttt{GLS} function would be much more
longer than \texttt{OLS} function. That's why parallelisation is used
for \texttt{GLS}function which is more demanding because of
\texttt{The\ Cholesky\ Decomposition}.

Lets run the simulation for \texttt{OLS} function first.
...
Now lets run it for \texttt{GLS} function with parallelisation and check
the execution time.On MacBook Air with 8 cores total execution time is
16.84 seconds.
...
As seen on the summary part, total execution time is 36.35 seconds which
also proves that parallel process works well.

Now, lets use the simulation results and visualise them by using ggplot2
methods.Here, additionaly \texttt{MSE}(Mean Square Error) are calculated
...
\includegraphics{Seminararbeit_files/figure-latex/unnamed-chunk-7-1.pdf}
...
\includegraphics{Seminararbeit_files/figure-latex/unnamed-chunk-8-1.pdf}
