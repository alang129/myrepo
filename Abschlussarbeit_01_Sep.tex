...
Monte Carlo studys simulate complex probabilistic events using simple
random events, such as tossing a pair of dice to simulate the casino's
...
?under study? \autocite{Barbu_2022}. Monte Carlo studys get combined
with programming in modern research and contributes to various studies
in statistics, economics, and many other science fields. The paper makes
a progress on developing a collection of different wrapper functions.
...
The paper proceeds as follows. Chapter 2 describes preprocesses to
establish the Monte Carlo simulation function. The preprocesses includes
functions to create grid and dataset with random variables in
user-defined distributions, along with functions that provide summary
statistics. Chanter 3 details the main Monte Carlo simulation function
which consists of functions in the chapter 2. Chapter 4 presents
examples with the main function. Finally, chapter 5 concludes. Each
chapter contains a simple example to show that a function is applicable
to as many cases as possible. If there is a restriction, It will be
covered and discussed as well.
...
end of sequence, the steps. It would be easy to adapt this helper
...
as inputs. The user can define a probability distribution of data by
entering the name of R packages into the function, f.e. the normal
distribution(\texttt{rnorm}) or uniform distribution(\texttt{runif}).

The n data points created for each set of parameters get stored as
separate elements in a list, since since format is a very flexible way
of storing data. \texttt{data\_generation()} chooses a mapping function
itself based on the number of parameters. The table below shows Mapping
function, Mapping function for parallelization and the number of
parameters that is used in \texttt{data\_generation()}:
...
\texttt{purrr}-function
\end{minipage} & \begin{minipage}[b]{\linewidth}\centering
equivalent \texttt{furrr}-function for parallelisation
\end{minipage} & \begin{minipage}[b]{\linewidth}\centering
...
Monte Carlo simulations can quickly become very demanding in terms of
computing time. In that case, it parallel processing is a way to reduce
the overall computing time. Processes are parallel if at any time both
of them are simultaneously executed, for instance, processes are
executed by separate, distributed processors interconnected by
...
with grid2 and \texttt{rpois()} (the Poisson distribution) as input. The
second example below uses a uniform distribution as underlying data
generating process, using the \texttt{runif()} function and a different
parameter list.
...
\NormalTok{                         ,}\FunctionTok{c}\NormalTok{(}\StringTok{"min"}\NormalTok{, }\DecValTok{0}\NormalTok{, }\DecValTok{1}\NormalTok{, }\FloatTok{0.5}\NormalTok{)}
\NormalTok{                         ,}\FunctionTok{c}\NormalTok{(}\StringTok{"max"}\NormalTok{, }\DecValTok{2}\NormalTok{, }\DecValTok{3}\NormalTok{, }\FloatTok{0.5}\NormalTok{))}
...
\FunctionTok{head}\NormalTok{(}\FunctionTok{data\_generation}\NormalTok{(}\AttributeTok{simulation=}\NormalTok{runif, }\AttributeTok{grid=}\NormalTok{grid\_unif), }\DecValTok{5}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
## 
## $n4
##  [1] 0.3267392 1.6327548 0.8587912 1.6418953 0.8009331 0.4692278 1.9557358
##  [8] 0.2339875 1.1669476 1.2787636
## 
## $n5
##  [1] 1.4999724 0.8320589 1.2215326 2.3861846 1.2072560 2.2258756 2.2860955
##  [8] 1.5218375 1.0267244 0.3677367
\end{verbatim}

Like explained before, the data gets stores in a list, where the data
points for each set of parameters are stored under a different variable
in the list. The first variable (saved under \texttt{\$n1}) are based on
10 draws from a uniform distribution with min = 0 and max = 2. Each
...
In the previous passages the process of creating \emph{raw} data has
been demonstrated by using \texttt{create\_grid()} and
\texttt{data\_generation()}. Based on the raw data, this sub-chapter
introduces another function called \texttt{summary\_function()}, which
calculates summary statistics that the user defines. The function uses
two inputs, namely \texttt{sum\_fun}, that is the summary function the
user wants to apply to the raw data. Secondly, it also uses the list
containing the raw data, that was introduced earlier.

The function basically applies the user-defined summary function
(\texttt{sum\_fun}) on the raw data using a \texttt{sapply()}-loop.
Results are stored in a \((nrow(grid ) * 1)\)-dimensional matrix, which
is combined with the parameter grid in the next step.
...
created from a normal distribution was used, with the respective
parameters n, \(\mu\) and the standard deviation specified in the
parameter list. The list containing the raw data using
\texttt{data\_generation()} is stored in the object \texttt{test\_data},
which is the data input for \texttt{summary\_function()}. The arithmetic
mean, using R´s built in \texttt{mean()} - function is used as second
input in order to calculate the mean of all data points, that were
created under a unique set of parameters. This output relates exactly to
each row of the used parameter grid. At a later point, these summary
statistics get merged with the parameter grid in a data frame.
...
Even tough a relatively small parameter grid was used in the example
above, the resulting parameter grid span over 180 rows. Storing the
simulation results in a data frame like previously mentioned offers some
benefits, since this way of storing the data allows the easy application
of further operations, f.e. filtering or grouping. But printing a data
frame containing many hundreds or thousands of rows may be not the most
overseeable option to displaying the data. A multi-dimensional array is
better suited for this purpose, since it organizes the output in a clear
and readable way.
...
In order to test this function, it is required to set up an slightly
altered version of the \texttt{main\_function()}, that is introduced in
the next passage. Also, a slightly modified version of the example using
the \texttt{rnorm()} is used, where the parameter grid spans over a
larger sequence.
...
## n=10  0.38852940 1.2958278 1.654964 2.873226 3.946540 5.001069 5.478657
## n=20  0.15109873 1.0393604 1.738710 2.914990 3.861910 4.803358 5.984248
## n=30 -0.07515184 1.3283913 2.215668 3.094384 3.970312 4.903682 5.901025
## n=40 -0.01810628 1.0151712 2.051085 3.149340 4.010517 5.048402 6.017574
## n=50 -0.23638102 1.1128823 1.887610 3.009564 3.846885 4.835838 5.903250
## n=60  0.10464633 0.9180269 2.190404 2.997401 4.043258 4.789295 5.941349
## 
## , , sd=2, rep=1
## 
##             mu=0      mu=1     mu=2     mu=3     mu=4     mu=5     mu=6
## n=10  0.46130810 1.0867712 1.787910 2.282957 3.428849 3.774930 6.406011
## n=20 -0.06915425 1.8991747 1.532334 2.589896 4.064915 4.984172 5.873684
## n=30 -0.67530122 1.0673304 1.950198 2.843340 3.647555 5.061126 6.282375
## n=40 -0.10123592 1.0168971 2.208097 3.486397 4.170538 4.012105 6.171029
## n=50  0.40896804 1.0650971 2.176621 2.769466 4.176131 5.167572 6.000878
## n=60 -0.47245393 0.8174299 2.153614 2.902508 4.189536 4.848555 5.788071
## 
## , , sd=1, rep=2
## 
##             mu=0      mu=1     mu=2     mu=3     mu=4     mu=5     mu=6
## n=10 -0.40134525 1.2244236 2.136922 3.161025 3.855949 4.944544 5.582184
## n=20  0.51775782 1.1909615 1.997094 3.078518 3.915702 5.067612 5.888923
## n=30  0.25972096 0.9920476 1.980259 2.771677 4.106929 4.863302 5.906029
## n=40  0.01759771 1.1986795 1.872412 2.859314 4.126803 5.091872 6.177908
## n=50  0.08154743 0.7448700 1.869296 3.119717 4.292366 5.157424 6.029218
## n=60  0.18014158 0.9006125 2.289024 3.073500 4.128961 5.111369 5.893950
## 
## , , sd=2, rep=2
## 
##             mu=0      mu=1     mu=2     mu=3     mu=4     mu=5     mu=6
## n=10  0.92832149 0.7263651 2.256106 3.854874 3.359467 4.820510 5.238463
## n=20 -0.36585639 1.6482716 1.656444 3.368212 3.776337 5.374825 6.332365
## n=30 -0.30692014 0.7061816 1.874429 2.724755 4.023220 5.518708 5.820588
## n=40 -0.01877822 0.9747193 2.260752 2.727071 4.282821 4.642044 5.600639
## n=50  0.52097723 0.6826509 1.812745 3.012439 4.168838 4.878543 5.729214
## n=60 -0.08386831 0.9961578 2.066676 3.051086 3.617138 4.881913 5.947985
\end{verbatim}

Under these specifications, the data was transformed into an
...
The output contains the correct amount of tables (4). This style of
output allows the user to easily oversee a broad variety of different
...
constellation, to keep the output simple. In praxis, the user would
probably repeat the simulations over a higher number of repitions, which
would also drastically increase the size of the array.
...
## 
##  Number of Cores Used in  Parallelization :  1  out of 4 
## 
...
##  Median  : 5 
## 
##  Execution Time of Monte Carlo Simulation 0.241014 secs 
## 
...
Also with \texttt{facet\_grid()} function.
...
##  Median  : 1.246948 
## 
##  Execution Time of Monte Carlo Simulation 0.1380081 secs 
## 
##  Name of The Class : Eco
\end{verbatim}
...
Same function ,\texttt{OLS} is now used in parallel with 4 cores.
Execution time of the simulation is 3.85 seconds.

Now lets run \texttt{GLS} function in parallel and check the execution
time. Execution time of the simulation is 16.84 seconds (with 4 cores).
...
## 
##  Number of Cores Used in  Parallelization :  4  out of 4 
## 
##  Input Parameters :  c("n", "100", "1000", "100") c("mu", "0", "1", "0.25") c("sd", "1", "2", "0.5") 
## 
##  Simulation Length : 750 
...
##  Median  : 1.284057 
## 
##  Execution Time of Monte Carlo Simulation 34.72199 secs 
## 
##  Name of The Class : Eco
\end{verbatim}
...
##  Median  : 1.284057 
## 
##  Execution Time of Monte Carlo Simulation 33.73493 secs 
## 
##  Name of The Class : Eco
\end{verbatim}
